...
/*
 * @brief 将sockaddr地址转成字符串地址
 *        转换结果形如：ipv4 "192.168.1.10:80" or "192.168.1.10"
 *                      ipv6 ""
 *                      域套接字 ""
 * @param sa 待转换的地址
 * @param socklen 	sa输入地址的长度
 * @param text  调用者分配的用于输出缓冲区
 * @param len	text指向的缓冲区的大小
 * @param port  指定输出地址是否包含端口
 * @return      实际输出大小
 */
size_t
